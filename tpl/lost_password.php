...
$url = str_replace('password_recovery', 'change_password', $_POST['url']);
$reset_link = home_url().$url.'&key='.$key.'&login='.rawurlencode($user_login).'&redirect_to='.esc_attr($redirect_to); // создадаем ссылку на сброс пароля, подразумевается что на странице с урлом /reset-password/ у вас будет форма залания нового пароля
$txt = '<h3>Доброго времени.</h3><p>Кто-то запросил сброс пароля на сайте: '.home_url().', чтобы сбросить пароль перейдите по ссылке: <a href="'.$reset_link.'">'.$reset_link.'</a>. Если произошла ошибка, просто проигнорируйте это письмо, и ничего не произойдёт..</p>'; // это текст письма
...
wp_mail( $user_email, 'Сброс пароля пользователя '.$user_login, $txt, $headers ); // отправляем письмо юзеру
